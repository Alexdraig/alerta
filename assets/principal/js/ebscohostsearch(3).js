...
******************************************************************************************************************************************************/
function _isEmpty2(e){
	return (e===null||typeof e==="undefined"||e===undefined);
} 
function ebscoHostSearchParse2(ebscohostsearchtext2,ebscohostsearchmode2) {
 var sT=ebscohostsearchtext2.match(/(\(|\)|[^"^\s^)]+)\s*|"([^"]+)"\s*/gi); 
 var sQ=''; 
 var bP=0; 
 var fP=0; 
 var nP=0;
 ...
 for(var i = 0;i<sT.length;i++) { 
 		...
 		switch (sT[i].toUpperCase()) {
 			...
 			case '&': case '+': break;
 			default: if (sT[i].toLowerCase().search(/^w([01]?\d\d?|2[0-4]\d|25[0-5])$/)!=-1||sT[i].toLowerCase().search(/^n([01]?\d\d?|2[0-4]\d|25[0-5])$/)!=-1) { sQ+='\+'+sT[i]+'\+'; bP=1; break; } else { if (sQ.length>nP && bP==0) { if (fP!=2) sQ+=ebscohostsearchmode2; else sQ+='\+'; } sT[i]=sT[i].replace(/\+/g,"%2b"); sQ+=sT[i]; bP=0; } } }
...
function ebscoHostSearchGo(form) {
	function ebscoAddDisciplines() {
		function addScrollBoxLimiters(disciplinesDiv) {
			...
			if (options.length) {
				...
				if (selectedOptionValues.length) {
					...
					ebscohosturl2 += '&cli' + index + '=DISCIPLINE' + '&clv' + index + '=';
					for (var i = 0; i< selectedOptionValues.length; i++) {
						ebscohosturl2 += 'LO+system.dis-' + selectedOptionValues[i].toLowerCase() + '%7e';
					}
				}
			...
			function addDisciplinesToUrl () {
				...
				function addToUrl (value) {
					ebscohosturl2 += 'LO+system.dis-' + value.toLowerCase() + '%7e';
				}
			}
		...
		if (disciplinesDiv) {
			...
			if (checkboxes.length) {
				...
				for (var i = 0; i < checkboxes.length; i++) {
					...
					code = checkbox.value;
					if (checkbox.checked) {
						ebscohosturl2 += 'LO+system.dis-' + code.toLowerCase() + '%7e';
					}
				}
		...
		ebscohosturl2 = ebscohosturl2.replace(/%7e$/, '');

	}


	 var ebscohostsearchtext2 = chkObject(form.ebscohostsearchtext2,'');
	 var ebscohostkeywords2 = chkObject(form.ebscohostkeywords2,'');
	 var ebscohostsearchsrc2 = chkObject(form.ebscohostsearchsrc2,'');
	 var ebscohostsearchmode2 = chkObject(form.ebscohostsearchmode2,'\+AND\+');
	 var ebscohostwindow2 = parseInt(chkObject(form.ebscohostwindow2,0));
	 var ebscohosturl2 = chkObject(form.ebscohosturl2,'http://search.ebscohost.com/login.aspx?');
	var isDMP = ebscohosturl2.indexOf('dynamed') > -1;
     var bquery = "bquery";
	 if (isDMP){
		 bquery = "q";
	 }
	...
	/* [7.5.2017]  Updated below to encodeURIComponent to allow for # wildcard to work in Permalink */
		ebscohostsearchtext2 = encodeURIComponent(ebscohostsearchtext2);
	
	 if (ebscohosturl2.indexOf('eds-live') == -1 && ebscohosturl2.indexOf('pfi-live') === -1) {
	 		if (ebscohostsearchsrc2 == "db" || ebscohostsearchsrc2 == "dbgroup") {
	 	 		var ebscohostdatabases = getSelectedDatabases(form.cbs,ebscohostsearchsrc2);
	 	 	if (ebscohostdatabases==-1) 
	 		 	strAlert+="Please select one or more databases.\n";
	 	 	
	 	 	ebscohosturl2+=ebscohostdatabases; 
		 	}
		}
	
	 if (ebscohostsearchtext2=="") 
	 ...
	 var matches=ebscohosturl2.match(/cli[0-9]/g);
	 var index = (matches===null)?0:matches.length; 
	 
	 if (!_isEmpty2(cbFT) && cbFT.checked) {	 	 
		ebscohosturl2	+= "&cli"+index+"=FT&clv"+index+"=Y";
		index++;
	 }	 
	 
	 var cbPR = document.getElementById("chkPeerReviewed");
	 	 
	 if (!_isEmpty2(cbPR) && cbPR.checked) {	 	 
		var rv = document.getElementById("ebscoIsPubFinder") ? "RV3" : "RV";
	 	ebscohosturl2 += "&cli" + index + "=" + rv + "&clv" + index + "=Y";
		 index++;
	 }	 
	 
	 var cbCO = document.getElementById("chkCatalogOnly");
	 	 
	 if (!_isEmpty2(cbCO) && cbCO.checked) {	 	 
		 ebscohosturl2+= "&cli"+index+"=FC&clv"+index+"=Y";
		 index++;
	 }
	 ...
	 if (!_isEmpty2(cbIRO) && cbIRO.checked) {	 	 
		 ebscohosturl2+= "&cli"+index+"=FC1&clv"+index+"=Y";
		 index++;
	 }
	 
	 var cbLC = document.getElementById("chkLibraryCollection");
	 	 
	 if (!_isEmpty2(cbLC) && cbLC.checked) {	 	 
		 ebscohosturl2+= "&cli"+index+"=FT1&clv"+index+"=Y";
		 index++;
		}
	...
	if (ebscohostkeywords2!="") 
		ebscohostkeywords2=ebscoHostSearchParse2(ebscohostkeywords2,ebscohostsearchmode2)+'\+AND\+';
	...
	var keywordSelector2 = document.getElementById("guidedField_0_2");
	var titleSelector2 = document.getElementById("guidedField_1_2");
	var authorSelector2 = document.getElementById("guidedField_2_2");
		
	// if(!_isEmpty2(authorSelector) && authorSelector.checked) {
	// 	ebscohosturl2+= '&bquery=AU+('+ebscohostkeywords2+ebscoHostSearchParse2(ebscohostsearchtext2,ebscohostsearchmode2)+')'; 
	// } else if(!_isEmpty(titleSelector) && titleSelector.checked) {
	// 	ebscohosturl2+= '&bquery=TI+('+ebscohostkeywords2+ebscoHostSearchParse2(ebscohostsearchtext2,ebscohostsearchmode2)+')'; 
	// } else {
	// 	ebscohosturl2 += '&'+bquery+'='+ebscohostkeywords2+ebscoHostSearchParse2(ebscohostsearchtext2,ebscohostsearchmode2);  	
	// }

	if(!_isEmpty2(authorSelector2) && authorSelector2.checked) {
		ebscohosturl2+= '&bquery=AU+('+ebscohostkeywords2+ebscoHostSearchParse2(ebscohostsearchtext2,ebscohostsearchmode2)+')'; 
	} else if(!_isEmpty2(titleSelector2) && titleSelector2.checked) {
		ebscohosturl2+= '&bquery=TI+('+ebscohostkeywords2+ebscoHostSearchParse2(ebscohostsearchtext2,ebscohostsearchmode2)+')'; 
	} else {
		ebscohosturl2 += '&'+bquery+'='+ebscohostkeywords2+ebscoHostSearchParse2(ebscohostsearchtext2,ebscohostsearchmode2);  	
	}
	...
	if (ebscohostwindow2)
		window.open(ebscohosturl2, 'EBSCOhost');
	else
		window.location = ebscohosturl2;
